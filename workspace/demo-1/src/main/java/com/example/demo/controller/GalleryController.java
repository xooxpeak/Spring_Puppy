...
public class GalleryController {
	...
	public void deleteGallery(@RequestParam(name = "id") Long id) {
	...
	// 다중 파일
//	@GetMapping(value = "/galleryView", produces={MediaType.IMAGE_PNG_VALUE, MediaType.IMAGE_JPEG_VALUE})
//	public ResponseEntity<List<byte[]>> findById(@RequestParam("id") Long id) throws IOException {
//		// id에 해당하는 이미지 정보를 서비스로부터 가져옴
//		GalleryEntity galleryEntity = galleryService.findById(id);
//		List<byte[]> images = new ArrayList<>();
//
//		// GalleryEntity에서 이미지 경록 목록을 가져옴
//		for (String imagePath : galleryEntity.getGallImg()) {
//			// 파일 경로를 이용하여 이미지 파일을 읽기 위한 InputStream을 생성
//			try (InputStream inputStream = new FileInputStream(imagePath)) {
//				// InputStream에서 모든 바이트를 읽어와 byte 배열에 저장
//				byte[] bytes = inputStream.readAllBytes();
//				// 이미지를 byte 배열로 변환하여 리스트에 추가
//				images.add(bytes);
//			}
//		}
//		// 이미지를 byte 배열 리스트로 변환하여 ResponseEntity에 담아서 반환
